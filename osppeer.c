 ...
 */
//Task 2: increasing taskbufsiz fixes "tracker connection closed prematurely" message
#define TASKBUFSIZ	50000 /*4096*/	// Size of task_t::buf
...
static task_t *task_listen(task_t *listen_task)
{
	...
	if (fd == -1 && (errno == EINTR || errno == EAGAIN
		...
		die("accept");
	char* ip = "164.67.100.232"; //this will change depending on who you want to block
	char *ip2 = inet_ntoa(peer_addr.sin_addr);
	if (strcmp(ip, ip2) == 0)
	{
		message("hello");
		exit(1);
...
static void task_upload(task_t *t)
{
	...
	/*EXERCISE 2
	First, make sure the peer does not serve files outside the current directory 
	*/
	//Stuff that tries to change directory path: ~,~/ at the beginning of the filename
	//second condition only happens when there are more than 1 characters in the filename 
	if (strlen(t->filename) > 1) { 
		if (t->filename[0] == '~' || (t->filename[0] == '~' && t->filename[1] == '/')) {
			//quit 
			error("Trying to access different directory(1)");
			goto exit;
		}
	}
	//Stuff that could happen anywhere in filename: .. (local path) 
	//check everywhere in the file 
	int i = 0; 
	
	for (i = 0; t->filename[i] != '\0'; i++) {
		//if current char = '.' and prev char = '.' then quit 
		if (t->filename[i] == '.' && t->filename[i-1] == '.') {
			error("Trying to access different directory(2)");
			goto exit; 
		}
	}
	//Now make sure we are in current directory 
	char requested_buffer[PATH_MAX];
	char current_buffer[PATH_MAX];
	char* real = realpath(t->filename,requested_buffer);
	char* current = getcwd(current_buffer, PATH_MAX);
	if ( !real || !current) {
		error("not in current directory!");
		goto exit; 
	}

	
	//Now doublecheck real and current are same length
	if (strncmp(real, current, strlen(current)) != 0) {
		error("Lengths are different");
		goto exit;
	}
...
/* EXERCISE 1:
	We need to split up tracker_test, let's grab every task in tracker_task first. Then we can parallelize each task, whether it be downloading or uploading.
*/
	//Instead of one tracker_task, make a tracker_task array with size argc 
	task_t *tracker_task_array[argc]; 
	pid_t pid;
	...
	if (argc == 1) {
		tracker_task_array[0] = start_tracker(tracker_addr, tracker_port);
	}
	
	for (i = 0; i < argc-1; i++) {
		//For each argc, add corresponding argc task 
		tracker_task_array[i] = start_tracker(tracker_addr, tracker_port); 
	}

	// Connect to the tracker and register our files.
	//tracker_task = start_tracker(tracker_addr, tracker_port);
	listen_task = start_listen();
	register_files(tracker_task_array[0], myalias);

	//PART 1: DOWNLOAD 
	for (i = 0; argc > 1; argc--, argv++) {
		pid = fork();
		if (pid == 0) {
			if ((t = start_download(tracker_task_array[i], argv[1])))
				task_download(t, tracker_task_array[i]);
		}
		else if (pid > 0) { 
			//skip 
		}
		else {
			//if we are here, then some error happened 
			fprintf(stderr, "\nError!\n");
			exit(1);
		}
	}
